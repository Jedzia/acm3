...
int WinMain (int argc, char *argv[])
{
  ...
  if (!win_openwindow("C64 Sprite Editor 2.0", NULL)) return 1;
  ...
  for (;;)
  {
    ...
    if (phase == 3)
    {
      ...
      if (r == 1)
      {
        ...
        fwrite8(handle, datasizehi);
        fwrite8(handle, datasizelo);
        for (c = 0; c < frames; c++)
        {
        ...
        for (c = 0; c < frames; c++)
        {
          ...
          for (slice = 0; slice < 9; slice++)
          {
            ...
            if (data) tempheader.slicemask |= (1 << slice);
          }
          tempheader.slicemask |= (spritedata[(frame+c)*64+63] << 9);
          if (magx[frame+c]) tempheader.slicemask |= (1 << 13);
          tempheader.hotspotx = hotspotx[frame+c];
          if (magx[frame+c])
            tempheader.reversehotspotx = 44-hotspotx[frame+c];
          else
            tempheader.reversehotspotx = 22-hotspotx[frame+c];
          tempheader.hotspoty = hotspoty[frame+c];
          tempheader.connectspotx = connectspotx[frame+c];
          if (magx[frame+c])
            tempheader.reverseconnectspotx = 44-connectspotx[frame+c];
          else
            tempheader.reverseconnectspotx = 22-connectspotx[frame+c];
          tempheader.connectspoty = connectspoty[frame+c];
          fwritele16(handle, tempheader.slicemask);
          fwrite8(handle, tempheader.hotspotx);
          fwrite8(handle, tempheader.reversehotspotx);
          fwrite8(handle, tempheader.hotspoty);
          fwrite8(handle, tempheader.connectspotx);
          fwrite8(handle, tempheader.reverseconnectspotx);
        ...
        datasizehi = datasize >> 8;
        datasizelo = datasize & 0xff;
        fwrite8(handle, datasizehi);
        fwrite8(handle, datasizelo);
